...
public class StructurizrWorkspaceLoader {
   ...
   *
   * <p>In Quarkus applications, logging is managed by JBoss LogManager which implements the
   * java.util.logging (JUL) API. Apache HttpClient 5 uses SLF4J for logging, which is bridged to
   * JBoss LogManager via slf4j-jboss-logmanager.
   *
   * <p>This method configures the underlying JUL loggers (which are actually JBoss LogManager
  ...
  public Workspace loadWorkspace() throws StructurizrClientException {
    ...
    if (config.isDebugMode()) {
      ...
      logger.debug(
          "Debug mode: API URL: {}",
          config.getApiUrl() != null ? config.getApiUrl() : "Structurizr Cloud");
      logger.debug(
          "Debug mode: API Key: {}...",
          config.getApiKey() != null && config.getApiKey().length() > 4
    ...
    try {
      ...
      if (config.isDebugMode()) {
        logger.debug(
            "Debug mode: HTTP request failed after {} ms", System.currentTimeMillis() - startTime);
    ...
    if (workspace == null) {
      if (config.isDebugMode()) {
        logger.debug(
            "Debug mode: HTTP request completed in {} ms but returned null workspace",
            endTime - startTime);
      }
    ...
    if (config.isDebugMode()) {
      ...
      logger.debug(
          "Debug mode: Workspace loaded - Name: '{}', ID: {}",
          workspace.getName(),
          config.getWorkspaceId());
      if (workspace.getModel() != null) {
        logger.debug(
            "Debug mode: Workspace contains {} software systems",
            workspace.getModel().getSoftwareSystems().size());
      }
    }

    logger.info(
        "Successfully loaded workspace '{}' (ID: {})",
        workspace.getName(),
        config.getWorkspaceId());
