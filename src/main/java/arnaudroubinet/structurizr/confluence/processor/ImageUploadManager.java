...
public class ImageUploadManager {
  ...
  /**
   * Downloads an external image and uploads it as an attachment to the specified page. Returns the
   * filename to use for referencing the attachment.
   *
  ...
  public String downloadAndUploadImage(String imageUrl, String pageId) throws IOException {
    ...
    try {
      ...
      // Try detailed upload first to get media identifiers; fallback to legacy upload if
      // unavailable
      MediaUploadResult result;
      try {
        ConfluenceClient.AttachmentDetails details =
            confluenceClient.uploadAttachmentDetailed(pageId, filename, imageContent, mimeType);
        if (details != null) {
          result =
              new MediaUploadResult(details.filename(), details.fileId(), details.collectionName());
          logger.info(
              "Successfully downloaded and uploaded image (detailed): {} -> {} (attachment ID: {}, fileId: {}, collection: {})",
              imageUrl,
              details.filename(),
          ...
          String attachmentId =
              confluenceClient.uploadAttachment(pageId, filename, imageContent, mimeType);
          result = new MediaUploadResult(filename, null, null);
          logger.info(
              "Successfully downloaded and uploaded image (fallback): {} -> {} (attachment ID: {} - no media IDs)",
              imageUrl,
              filename,
              attachmentId);
        }
        ...
        logger.debug(
            "uploadAttachmentDetailed failed, falling back to uploadAttachment: {}",
            detailedEx.toString());
        String attachmentId =
            confluenceClient.uploadAttachment(pageId, filename, imageContent, mimeType);
        result = new MediaUploadResult(filename, null, null);
  ...
  public String uploadLocalFile(File localFile, String pageId) throws IOException {
    ...
    try {
      ...
      try {
        ConfluenceClient.AttachmentDetails details =
            confluenceClient.uploadAttachmentDetailed(pageId, filename, fileContent, mimeType);
        if (details != null) {
          result =
              new MediaUploadResult(details.filename(), details.fileId(), details.collectionName());
          logger.info(
              "Successfully uploaded local file (detailed): {} to page {} (fileId: {}, collection: {})",
              filename,
              pageId,
              details.fileId(),
              details.collectionName());
          ...
          result = new MediaUploadResult(filename, null, null);
          logger.info(
              "Successfully uploaded local file (fallback): {} to page {} (attachment ID: {} - no media IDs)",
              filename,
              pageId,
              attachmentId);
        }
        ...
        logger.debug(
            "uploadAttachmentDetailed failed for local file, falling back: {}",
            detailedEx.toString());
        String attachmentId =
            confluenceClient.uploadAttachment(pageId, filename, fileContent, mimeType);
        result = new MediaUploadResult(filename, null, null);
  ...
  /**
   * Simple carrier for media identifiers required by ADF media nodes. filename is still provided
   * for fallback when media ids are missing.
   */
