...
/**
 * Export command that exports a Structurizr workspace to Confluence. Replicates the functionality
 * of ConfluenceExporterIntegrationTest.
 */
@CommandLine.Command(
    name = "export",
...
public class ExportCommand implements Runnable {
  ...
  @CommandLine.Option(
      ...
      description =
          "Confluence space key (default: CONFLUENCE_SPACE_KEY env var, required when using --page-title)",
      required = false)
  ...
  @CommandLine.Option(
      ...
      description =
          "Path to Structurizr workspace JSON file (takes priority over Structurizr options)",
      required = false)
  File workspaceFile;
  ...
  public void run() {
    try {
      ...
      // Export workspace
      logger.info(
          "Starting workspace export with parent page ID: {} and branch: {}", pageId, branchName);
      if (workspace != null) {
  ...
  private boolean validateConfiguration() {
    ...
    // Space is required for normal export operations unless using page-id which can work without
    // space
  ...
  private boolean promptForCleanConfirmation(String targetPageTitle, String targetPageId) {
    ...
    System.out.printf(
        "⚠️  The --clean option will delete the %s%s and ALL its subpages before export.%n",
        target, spaceInfo);
