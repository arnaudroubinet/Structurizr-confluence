...
/**
 * Integration test to verify que l'extraction de titre fonctionne correctement in the complete
 * context de l'export Confluence.
 */
class TitleExtractionIntegrationTest {
  private static final Logger logger =
      LoggerFactory.getLogger(TitleExtractionIntegrationTest.class);
  ...
  void testCompleteExportProcessWithTitleExtraction() {
    ...
    assert !adfJson.contains(
            "\"text\" : \"ITMS : Instant Ticket Manager System - Introduction and Goals\"")
    ...
    assert adfJson.contains(
            "\"text\" : \"This document describes the introduction and goals of the ITMS system.\"")
        : "Le paragraphe principal devrait être présent";

    logger.info(
        "✅ SUCCÈS : Le processus complet d'export avec extraction de titre works correctly");
  ...
  void testFallbackToFileNameWhenWhitespaceOnlyH1() {
    ...
    logger.info(
        "✅ SUCCÈS : Le fallback au nom de fichier works correctly quand le H1 est vide/espaces");
  }
}
