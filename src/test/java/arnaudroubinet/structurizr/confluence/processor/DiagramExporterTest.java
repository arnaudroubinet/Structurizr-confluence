...
class DiagramExporterTest {
  ...
  void testFromEnvironmentWithoutVariables() {
    ...
    logger.info(
        "✅ DiagramExporter correctly returns null when STRUCTURIZR_* variables are not configured");
  }
  ...
  void testFromEnvironmentWithVariables() {
    ...
    DiagramExporter exporter =
        new DiagramExporter("https://structurizr.roubinet.fr", "testuser", "testpass", workspaceId);
    ...
    assertTrue(
        exporter.getOutputDirectory().toString().contains("target/diagrams"),
  ...
  void testOutputDirectoryCreation() {
    ...
    DiagramExporter exporter =
        new DiagramExporter("https://structurizr.roubinet.fr", "testuser", "testpass", "123");
