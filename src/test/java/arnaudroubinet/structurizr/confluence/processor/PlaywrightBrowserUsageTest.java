...
/**
 * Test to verify that the application only uses Chromium browser from Playwright.
 * This ensures we only need to install chromium, not all browsers (firefox, webkit).
 * 
 * Related issue: "Playwright load many browsers, why? I only need chromium when loading my schemas."
 */
class PlaywrightBrowserUsageTest {
    ...
    void testOnlyChromiumIsUsedInDiagramExporter() throws IOException {
        ...
        Path sourceFile = Paths.get("src/main/java/arnaudroubinet/structurizr/confluence/processor/DiagramExporter.java");
        ...
        logger.info("   This means we can install with: mvn exec:java -D exec.mainClass=com.microsoft.playwright.CLI -D exec.args=\"install --with-deps chromium\"");
    ...
    void testGitHubWorkflowUsesChromiumOnly() throws IOException {
        ...
        assertTrue(workflowContent.contains("install --with-deps chromium"), 
            "GitHub Actions workflow should install only chromium browser");
        
        // Verify that it doesn't install all browsers (which would be "install --with-deps" without browser name)
        assertFalse(workflowContent.matches(".*install --with-deps\"\\s*$.*"), 
