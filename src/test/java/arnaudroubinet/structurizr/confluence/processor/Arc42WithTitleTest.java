...
class Arc42WithTitleTest {
  ...
  void testArc42ContentWithH1Title() {
    ...
    // Vérifications
    assert pageTitle.equals("Architecture Documentation")
        : "Le titre devrait être 'Architecture Documentation'";
    assert !adfJson.contains("Architecture Documentation")
        : "Le titre H1 ne devrait pas être dans l'ADF";
    ...
    assert adfJson.contains("\"href\" : \"https://arc42.org/overview\"")
        : "L'URL du lien devrait être dans les attrs";
    assert !adfJson.contains("arc42 (https://arc42.org/overview)")
        : "Le format fallback ne devrait plus être utilisé";
    ...
    logger.info(
        "✅ SUCCÈS : Titre extrait, H1 supprimé, liens natifs ADF générés, structure maintenue");
  }
}
