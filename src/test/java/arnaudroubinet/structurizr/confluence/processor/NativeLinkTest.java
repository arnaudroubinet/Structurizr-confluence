...
public class NativeLinkTest {
  ...
  void testMultipleLinksInParagraph() throws Exception {
    ...
    String html =
        "<p>Visit <a href=\"https://atlassian.com\">Atlassian</a> or <a href=\"https://github.com\">GitHub</a>.</p>";
    ...
    assertEquals(
        "https://atlassian.com", link1.get("marks").get(0).get("attrs").get("href").asText());
  ...
  void testArc42LinkExample() throws Exception {
    ...
    String html =
        "<p>This architecture document follows the <a href=\"https://arc42.org/overview\">arc42</a> template.</p>";
    ...
    String fullText =
        paragraphContent.get(0).get("text").asText()
            + paragraphContent.get(1).get("text").asText()
            + paragraphContent.get(2).get("text").asText();
    assertFalse(
        fullText.contains("(https://"), "Le lien ne devrait pas contenir l'URL en format fallback");
