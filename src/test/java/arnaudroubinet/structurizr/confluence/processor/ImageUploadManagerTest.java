...
class ImageUploadManagerTest {
  ...
  void testUploadCaching() throws Exception {
    ...
    when(confluenceClient.uploadAttachment(
    ...
    verify(confluenceClient, times(1))
        .uploadAttachment(eq(pageId), eq(filename1), any(byte[].class), anyString());
  ...
  void testImageHandlingWithUploadManager() throws Exception {
    ...
    String htmlWithExternalImage =
        "<img src=\"https://structurizr.roubinet.fr/workspace/1/diagrams/context-view.svg\" alt=\"Context Diagram\">";
    ...
    assertTrue(
        ...
        "External image should be uploaded and referenced as file");
    ...
    verify(confluenceClient, times(1))
        .downloadImage("https://structurizr.roubinet.fr/workspace/1/diagrams/context-view.svg");
    verify(confluenceClient, times(1))
        .uploadAttachment(
            eq("test-page-123"), eq("context-view.svg"), any(byte[].class), eq("image/svg+xml"));
