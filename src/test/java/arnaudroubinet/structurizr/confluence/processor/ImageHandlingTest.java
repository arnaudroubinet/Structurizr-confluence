...
class ImageHandlingTest {
  ...
  void testExternalImageHandling() throws Exception {
    ...
    String htmlWithExternalImage =
        "<img src=\"https://structurizr.roubinet.fr/workspace/1/diagrams/itms-context.svg\" alt=\"Context Diagram\" title=\"ITMS Context View\">";
    ...
    assertTrue(
        adfJson.contains("\"type\" : \"mediaGroup\""),
        "Should create mediaGroup for external image");
    assertTrue(
        adfJson.contains("\"type\" : \"media\""), "Should create media node inside mediaGroup");
    assertTrue(
        adfJson.contains("\"type\" : \"file\""), "External image should be uploaded as attachment");
    // URL might be different as it's uploaded as attachment, but should have the image name
    assertTrue(
        adfJson.contains("itms-context.svg") || adfJson.contains("attachment"),
        "Should reference uploaded image");
    assertTrue(
        adfJson.contains("\"occurrenceKey\" : \"ITMS Context View\""),
        "Should use title as caption");
  ...
  void testLocalImageHandling() throws Exception {
    ...
    assertTrue(
        adfJson.contains("\"type\" : \"mediaGroup\""), "Should create mediaGroup for local image");
    assertTrue(
        adfJson.contains("\"type\" : \"media\""), "Should create media node inside mediaGroup");
    ...
    assertTrue(
        adfJson.contains("\"occurrenceKey\" : \"Architecture Diagram\""),
        "Should use alt as caption");
  ...
  void testImageWithoutCaptionHandling() throws Exception {
    ...
    assertTrue(
        adfJson.contains("\"type\" : \"mediaGroup\""),
        "Should create mediaGroup even without caption");
    assertTrue(adfJson.contains("\"type\" : \"media\""), "Should create media node");
    assertFalse(
        adfJson.contains("\"occurrenceKey\""), "Should NOT have occurrenceKey when no caption");
  ...
  void testStructurizrDiagramEmbedWorkflow() throws Exception {
    ...
    assertTrue(
        adfJson.contains("\"type\" : \"mediaGroup\""),
        "Should preserve Structurizr diagram as media");
    assertTrue(
        adfJson.contains("\"type\" : \"file\""),
        "Structurizr diagram should be uploaded as attachment");
    assertTrue(
        adfJson.contains("context-view.svg") || adfJson.contains("attachment"),
        "Should reference uploaded diagram");
