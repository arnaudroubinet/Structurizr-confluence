...
/**
 * Test for the attachment replacement functionality in ConfluenceClient. This test validates the
 * logic that checks for existing attachments and updates them instead of creating duplicates.
 *
 * <p>Note: This is a unit test that validates the expected behavior. Full integration testing
 * requires a real Confluence instance.
 */
class AttachmentReplacementTest {
  ...
  void testDiagramFilenamePatterns() {
    ...
    for (String filename : diagramFilenames) {
      ...
      assertFalse(filename.contains("\\"), "Filename should not contain path separators");
      assertTrue(
          filename.matches("^[a-zA-Z0-9_.-]+$"), "Filename should only contain safe characters");
  ...
  void testEndpointDifferences() {
    ...
    logger.info(
        "   - UPDATE: POST /rest/api/content/{pageId}/child/attachment/{attachmentId}/data");
