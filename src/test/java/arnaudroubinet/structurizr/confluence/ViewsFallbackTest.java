...
/**
 * Test for Views page fallback mechanism when local diagrams aren't available. Validates that the
 * Views page uses workspace views with external URLs as a fallback.
 */
public class ViewsFallbackTest {
  ...
  public void testMultipleViewsFallback() throws Exception {
    ...
    SystemContextView contextView =
        views.createSystemContextView(system, "SystemContext", "System Context View");
    contextView.addAllElements();
    ...
    logger.info(
        "Created workspace with {} system context views and {} container views",
  ...
  public void testDiagramUrlBuilding() {
    ...
    String expectedUrl =
        structurizrUrl + "/workspace/" + workspaceId + "/diagrams/" + viewKey + ".svg";
    String actualUrl =
        structurizrUrl + "/workspace/" + workspaceId + "/diagrams/" + viewKey + ".svg";

    assertEquals(expectedUrl, actualUrl, "Diagram URL should be built correctly");
    assertEquals(
        "https://structurizr.example.com/workspace/123/diagrams/SystemContext.svg", actualUrl);
