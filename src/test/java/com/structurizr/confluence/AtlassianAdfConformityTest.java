...
/**
 * Test de conformité ADF selon la spécification officielle Atlassian.
...
public class AtlassianAdfConformityTest {
    ...
    private void validateRootDocumentStructure(JsonNode rootNode) {
        // Validation selon la spec: https://developer.atlassian.com/cloud/jira/platform/apis/document/structure/
        assertEquals(1, rootNode.get("version").asInt(), 
                "La version du document ADF doit être 1");
        assertEquals("doc", rootNode.get("type").asText(), 
                "Le type racine doit être 'doc'");
        assertTrue(rootNode.has("content"), 
                "Le document doit avoir un champ 'content'");
        assertTrue(rootNode.get("content").isArray(), 
                "Le 'content' doit être un array");
    }
    ...
    private void validateTableStructure(JsonNode tableNode) {
        ...
        for (JsonNode row : content) {
            assertEquals("tableRow", row.get("type").asText(), 
                    "Chaque ligne doit être de type 'tableRow'");
            ...
            for (JsonNode cell : rowContent) {
                ...
                assertTrue(cellType.equals("tableHeader") || cellType.equals("tableCell"), 
    ...
    private void validateHeadingStructure(JsonNode headingNode) {
        assertEquals("heading", headingNode.get("type").asText(), 
                "Le type doit être 'heading'");
        ...
        assertTrue(level >= 1 && level <= 6, 
                "Le niveau de heading doit être entre 1 et 6");
    ...
    private void validateParagraphStructure(JsonNode paragraphNode) {
        assertEquals("paragraph", paragraphNode.get("type").asText(), 
                "Le type doit être 'paragraph'");
        ...
        for (JsonNode textNode : content) {
            ...
            assertTrue(nodeType.equals("text") || nodeType.equals("strong") || nodeType.equals("em"), 
