...
/**
 * Test to validate that all Structurizr formatting is preserved in Confluence.
 * This addresses the issue: "Respect de la mise en forme structurizr dans confluence"
 */
class StructurizrFormattingValidationTest {
    private static final Logger logger = LoggerFactory.getLogger(StructurizrFormattingValidationTest.class);
    ...
    void testStructurizrWorkspaceFormattingPreservation() {
        ...
        try {
            ...
            assertTrue(adfJson.contains("\"type\" : \"link\""), "Les liens doivent être préservés avec des marks link natives");
            assertTrue(adfJson.contains("\"href\" : \"https://arc42.org/overview\""), "L'URL du lien doit être préservée");
            assertTrue(adfJson.contains("\"type\" : \"code\""), "Le formatage code doit être préservé");
            assertTrue(adfJson.contains("\"type\" : \"strong\""), "Le formatage strong doit être préservé");
            ...
            assertFalse(adfJson.contains("arc42 (https://arc42.org/overview)"), "Les liens ne doivent pas être convertis en format fallback");
            
            // Vérifier la structure des headings
            assertTrue(adfJson.contains("\"type\" : \"heading\""), "Les titres doivent être préservés");
            assertTrue(adfJson.contains("\"level\" : 2"), "Les niveaux de titre H2 doivent être préservés");
            assertTrue(adfJson.contains("\"level\" : 3"), "Les niveaux de titre H3 doivent être préservés");
            
            logger.info("✅ Tous les éléments de formatage Structurizr sont correctement préservés dans Confluence ADF!");
    ...
    void testComplexInlineFormattingMix() {
        ...
        try {
            ...
            logger.info("Marks détectées - Links: {}, Code: {}, Strong: {}, Em: {}", 
                linkCount, codeCount, strongCount, emCount);
