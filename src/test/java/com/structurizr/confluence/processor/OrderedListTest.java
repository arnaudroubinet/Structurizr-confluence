...
public class OrderedListTest {
    ...
    public void testOrderedListSupport() throws JsonProcessingException {
        ...
        Document bulletDocument = Document.create()
            .bulletList(list -> {
                list.item(item -> item.paragraph("Item 1"));
                list.item(item -> item.paragraph("Item 2"));
            });
            
        String bulletJson = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(bulletDocument);
        logger.info("BulletList JSON: {}", bulletJson);
        
        // TEST ORDERED LIST !!!
        Document orderedDocument = Document.create()
            .orderedList(list -> {
        ...
        String orderedJson = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(orderedDocument);
        logger.info("OrderedList JSON: {}", orderedJson);
        
        // TEST BLOCKQUOTE !!!
        Document quoteDocument = Document.create()
            .quote(quote -> {
                quote.paragraph("Ceci est une citation importante pour illustrer un point.");
            });
            
        String quoteJson = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(quoteDocument);
